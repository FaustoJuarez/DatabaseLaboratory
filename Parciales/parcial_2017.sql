...
-- Schema Parcial_2017
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `Parcial_2017` DEFAULT CHARACTER SET utf8 ;
USE `Parcial_2017` ;

-- -----------------------------------------------------
-- Table `Parcial_2017`.`Clientes`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Clientes` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Clientes` (
...
CREATE INDEX `IX_Telefono` ON `Parcial_2017`.`Clientes` (`Telefono` ASC) VISIBLE;

CREATE UNIQUE INDEX `Telefono_UNIQUE` ON `Parcial_2017`.`Clientes` (`Telefono` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`Categorias`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Categorias` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Categorias` (
  `IdCategoria` INT NOT NULL,
  `Nombre` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`IdCategoria`))
ENGINE = InnoDB;

CREATE INDEX `IX_Nombre` ON `Parcial_2017`.`Categorias` (`Nombre` ASC) VISIBLE;

CREATE UNIQUE INDEX `Nombre_UNIQUE` ON `Parcial_2017`.`Categorias` (`Nombre` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`Productos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Productos` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Productos` (
  ...
  CONSTRAINT `fk_Productos_Categorias1`
    ...
    REFERENCES `Parcial_2017`.`Categorias` (`IdCategoria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `IX_Nombre` ON `Parcial_2017`.`Productos` (`Nombre` ASC) VISIBLE;

CREATE INDEX `fk_Productos_Categorias1_idx` ON `Parcial_2017`.`Productos` (`IdCategoria` ASC) VISIBLE;

CREATE UNIQUE INDEX `Nombre_UNIQUE` ON `Parcial_2017`.`Productos` (`Nombre` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`Ventas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Ventas` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Ventas` (
  ...
  CONSTRAINT `fk_Ventas_Clientes`
    ...
    REFERENCES `Parcial_2017`.`Clientes` (`IdCliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `IX_Fecha` ON `Parcial_2017`.`Ventas` (`Fecha` ASC) VISIBLE;

CREATE INDEX `fk_Ventas_Clientes_idx` ON `Parcial_2017`.`Ventas` (`IdCliente` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`Ofertas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Ofertas` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Ofertas` (
...
CREATE INDEX `IX_FechaInicio` ON `Parcial_2017`.`Ofertas` (`FechaInicio` ASC) INVISIBLE;

CREATE INDEX `IX_FechaFin` ON `Parcial_2017`.`Ofertas` (`FechaFin` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`OfertasDelProducto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`OfertasDelProducto` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`OfertasDelProducto` (
  `IdProducto` INT NOT NULL,
  `IdOferta` INT NOT NULL,
  PRIMARY KEY (`IdProducto`, `IdOferta`),
  CONSTRAINT `fk_Productos_has_Ofertas_Productos1`
    FOREIGN KEY (`IdProducto`)
    REFERENCES `Parcial_2017`.`Productos` (`IdProducto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Productos_has_Ofertas_Ofertas1`
    FOREIGN KEY (`IdOferta`)
    REFERENCES `Parcial_2017`.`Ofertas` (`IdOferta`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_Productos_has_Ofertas_Ofertas1_idx` ON `Parcial_2017`.`OfertasDelProducto` (`IdOferta` ASC) VISIBLE;

CREATE INDEX `fk_Productos_has_Ofertas_Productos1_idx` ON `Parcial_2017`.`OfertasDelProducto` (`IdProducto` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `Parcial_2017`.`Detalles`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Parcial_2017`.`Detalles` ;

CREATE TABLE IF NOT EXISTS `Parcial_2017`.`Detalles` (
  ...
  CONSTRAINT `fk_Detalles_Ventas1`
    FOREIGN KEY (`IdVenta`)
    REFERENCES `Parcial_2017`.`Ventas` (`IdVenta`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Detalles_OfertasDelProducto1`
    FOREIGN KEY (`IdProducto` , `IdOferta`)
    REFERENCES `Parcial_2017`.`OfertasDelProducto` (`IdProducto` , `IdOferta`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_Detalles_Ventas1_idx` ON `Parcial_2017`.`Detalles` (`IdVenta` ASC) VISIBLE;

CREATE INDEX `fk_Detalles_OfertasDelProducto1_idx` ON `Parcial_2017`.`Detalles` (`IdProducto` ASC, `IdOferta` ASC) VISIBLE;
...
comprobaciones necesarias y devolver los mensajes de error correspondiente [15].
*****************************************************************************************/
...
BEGIN
    ...
    IF pNombre IS NULL OR trim(pNombre) = '' OR pPrecio IS NULL THEN
        SET pMensaje = 'Error Nombre ni Precio pueden ser nulos';
        LEAVE FINAL;
    END IF;
    ...
    IF EXISTS(SELECT Nombre FROM Productos WHERE Nombre = pNombre) THEN
        SET pMensaje = 'No puede haber nombres de productos repetidos';
        LEAVE FINAL;
    END IF;
    
    IF pPrecio IS NOT NULL AND pPrecio < 0.0 THEN
        SET pMensaje= 'Precio no puede ser nulo';
        LEAVE FINAL;
    END IF ;
...
CALL sp_CargarProducto('PeachiDev', NULL, 4.99,34, @pMensaje);
CALL sp_CargarProducto('MauriDev', NULL, 4.99,NULL, @pMensaje);
...
SELECT Ventas.IdVenta, Ventas.Fecha, Clientes.Apellidos, Productos.Nombre, Categorias.Nombre, Detalles.Cantidad,Productos.Precio
	FROM Ventas JOIN Clientes
	ON Ventas.IdCliente = Clientes.IdCliente
	JOIN Detalles
	ON Ventas.IdVenta = Detalles.IdVenta
	JOIN OfertasDelProducto
	ON Detalles.IdProducto = OfertasDelProducto.IdProducto
    JOIN Productos
    ON OfertasDelProducto.IdProducto = Productos.IdProducto
...
(SELECT null,
 ...
 FROM Ventas V
          ...
          LEFT JOIN Categorias C2 on P.IdCategoria = C2.IdCategoria)
;
/* 4) Realizar un trigger llamado AuditarOfertas el cual se dispare luego de insertar una oferta cuyo
...
DROP TABLE IF EXISTS `AuditorOfertas` ;

CREATE TABLE IF NOT EXISTS `AuditorOfertas` (
...
CALL AuditarOfertas (17, 10.0, '2005-06-01 00:00:00.000', '2008-12-31 00:00:00.000', 0, NULL);
